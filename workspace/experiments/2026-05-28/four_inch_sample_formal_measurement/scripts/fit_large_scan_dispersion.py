...
FAMILY_LABELS = {
    "deep_lower": "deep branch: lower folded-freq",
    "deep_upper": "deep branch: upper folded-freq",
    "side_mid": "side branch: lowest folded-freq",
}
...
def split_families(rows: list[dict[str, float]]) -> dict[str, list[dict[str, float]]]:
    ...
    for mode, mode_rows in by_mode.items():
        ...
        if len(mode_rows) >= 3:
            families["side_mid"].append(mode_rows[0])
            families["deep_lower"].append(mode_rows[1])
            families["deep_upper"].append(mode_rows[-1])
        elif mode_rows:
            ...
            for row in rows:
                ...
                for field in fields:
                    if field == "family":
                        ...
                        output[field] = FAMILY_LABELS.get(family, family)
                    else:
...
def plot_fits(
    ...
    ax.text(
        0.02,
        0.03,
        "Branch labels are assigned on the folded-frequency axis within each mode bin.",
...
def plot_auto_centered_fits(
    ...
    for ax, fit in zip(axes, visible_fits):
        ...
        ax.set_title(
            f"{FAMILY_LABELS.get(name, name)}: offset={float(fit['auto_offset_mhz'])/1000:.3f} GHz, "
...
def main(argv: Iterable[str]) -> int:
    ...
    if args.dip_table:
        ...
        dip_table = max(result_dir.glob("large_scan_*_dip_table.csv"), key=lambda item: item.stat().st_mtime)
