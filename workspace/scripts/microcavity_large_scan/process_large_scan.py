...
def find_transmission_dips(
    ...
    *,
    ...
    trans, baseline = normalize_transmission_with_baseline(trans_raw)
    ...
    if len(extrema) == 0:
        return np.array([], dtype=int), np.array([], dtype=float), trans, baseline
    ...
    return dips[:, 0].astype(int), dips[:, 1].astype(float), trans, baseline


def merge_narrow_extrema(trace: np.ndarray, extrema: np.ndarray, width: int) -> np.ndarray:
    if len(extrema) <= 1:
        return extrema
    window = max(3, int(width) * 2 + 1)
    local_min = minimum_filter1d(trace, size=window, mode="nearest")
    local_max = maximum_filter1d(trace, size=window, mode="nearest")
    indices = extrema[:, 0].astype(int)
    is_dip = extrema[:, 2] < 0
    is_peak = extrema[:, 2] > 0
    keep = np.zeros(len(extrema), dtype=bool)
    keep[is_dip] = extrema[is_dip, 1] <= local_min[indices[is_dip]] + 1e-12
    keep[is_peak] = extrema[is_peak, 1] >= local_max[indices[is_peak]] - 1e-12
    return extrema[keep]
...
def plot_envelope(
    ...
    *,
    ...
    """Plot a bin min/max envelope so narrow dips are not hidden by stride aliasing."""
    bin_size = max(1, math.ceil(len(time_s) / target_bins))
    starts = np.arange(0, len(time_s), bin_size)
    ends = np.minimum(starts + bin_size - 1, len(time_s) - 1)
    t_mid = 0.5 * (time_s[starts] + time_s[ends])
    y_min = np.minimum.reduceat(values, starts)
    y_max = np.maximum.reduceat(values, starts)
    ax.fill_between(t_mid, y_min, y_max, color=color, alpha=alpha, linewidth=0)
    ax.plot(t_mid, y_max, color=color, lw=lw, label=label)
...
def plot_results(
    *,
    ...
    plot_envelope(axes[0], time_s, trans_raw, target_bins=20000, color="#2364aa", label="CH2 raw envelope")
    if len(dip_idx):
    ...
    axes[0].set_title(f"{stem}: raw CH2/CH3 min-max envelope", fontsize=20)
    plot_envelope(axes[1], time_s, mzi_raw, target_bins=20000, color="#55a868", label="CH3 MZI envelope", lw=0.75)
...
def plot_normalized_transmission(
    *,
    ...
    bin_size = max(1, math.ceil(len(time_s) / 25000))
    t_plot = time_s[::bin_size]
    baseline_plot = baseline[::bin_size]

    fig, axes = plt.subplots(2, 1, figsize=(12, 6.6), sharex=True, constrained_layout=True)
    plot_envelope(axes[0], time_s, trans_raw, target_bins=25000, color="#2364aa", label="CH2 raw envelope", lw=0.55)
    ...
    plot_envelope(
        ...
        target_bins=25000,
        color="#2b8c5a",
        label="normalized + smoothed CH2 envelope",
        lw=0.6,
    )
    if len(dip_idx):
...
def parse_args(argv: Iterable[str]) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        default=None,
        help=f"Output directory. Defaults to the input data directory, or ${DATA_ROOT_ENV}/experiments/... when data_path is omitted.",
    )
