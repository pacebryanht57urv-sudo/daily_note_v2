...
- 短期、一次性或协作者平台复用 session 可建在 `DAILY_NOTE_DATA_ROOT` 下的外部 `experiments/YYYY-MM-DD/<session-name>/`。
- 跨多天推进的长期实验任务 / campaign 应按实验内容组织，例如 `experiments/<campaign-name>/daily/`、`results/`、`figures/`、`scripts/`；日期只作为 `daily/YYYY-MM-DD.md` 的进展索引，不作为数据组织的根。
- 每个外部 session 或 campaign 可按需包含 `session.md`、`daily/`、`figures/`、`images/`、`results/` 和外部数据路径说明。若当前 campaign 已改为内容主轴，`session.md` 只作为 legacy/history，不作为主要现场记录。
- 密集现场记录阶段，Codex 默认先在对话中轻量归类和必要追问；待用户明确说“整理一下 / 写进 daily / 今天收口 / 落盘”，或一段内容明显结束后，再批量更新外部记录。
- 每采完一组新测量数据后，Codex 应先和用户一起判断规律、有效性、异常点和下一步意义；确认有效或明确标记无效后，再写入对应物理对象的记录、卡片或 campaign 进展索引。
...
- 对用户给出的现场操作请求，Codex 必须按请求的最小必要范围执行；不能为了“整理得更完整”“让记录更自洽”擅自升级为重跑分析、重采数据、批量标准化、批量改路径或重构流程。
- 涉及已有实验数据、测量结果、图片、`.npz`、`.mat`、`.png`、`.jpg`、`.csv`、`.json` 等文件迁移或整理时，默认采用保守策略：先复制或移动，再做只读校验和少量记录更新；除非用户明确要求，不得重跑采集或分析，不得覆盖已有有效数据。
- 执行过程中一旦发现实际状态与用户描述不一致，或需要覆盖、删除、重跑、修复损坏文件、解释不确定数据、迁移到非空目录、修改二进制/图片/原始数据，Codex 必须立即停下来说明发现的问题，并向用户确认下一步；不得边猜边继续补救。
- 对二进制原始数据和图片文件只允许做文件级移动、复制、重命名、打开校验和哈希/大小检查；不得用文本读写、字符串替换、`Set-Content`、`Get-Content` 文本模式或类似方式处理 `.npz`、`.mat`、`.png`、`.jpg` 等二进制文件。
- 批量文本替换只能用于明确列出的文本文件，并应优先使用结构化读写 JSON/CSV/Markdown 的方式；不得对混合文件目录做宽泛递归文本替换。
